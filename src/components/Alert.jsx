...
const Alert = ({alertStyle, alertMode, handleAlert, currentRoute, currentMediaSet, handleAlertAction}) => {
    return (
        <div
            className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-500 h-200 flex 
            justify-center items-center
            rounded text-lg text-white text-center z-50
             ${alertStyle === alertSuccessStyle ? 'bg-emerald-400' : alertStyle === alertWarningStyle ?
                'bg-amber-200 text-black' : 'bg-red-700'}`}>
            <button onClick={() => handleAlert({overlayMode: true, toggle: false})}
                    className={'absolute top-0 right-0'}><AiOutlineClose/></button>
            {alertMode === alertRemoveAll &&
                <div>{removeAllMsg}</div>}
            {alertMode === alertRemoveAll &&
                < button onClick={() => handleAlertAction({alertMode, currentRoute, currentMediaSet})}
                         className={'absolute bottom-5'}>Ok</button>}
        </div>
    );
};
